...
def egg(M, T_0, T_y):
	...
	t = M**(2.0/3)*c*rho**(1.0/3)/(K*pi**2*(4*pi/3)**(2.0/3))*log(0.76*(T_0 - T_w)/(T_y - T_w))
	return t
...
print 'From the fridge: to soft boil a small egg cook for %.2f minutes and a large egg for %.2f minutes.' % (t_small/60, t_large/60)
...
M = 0.067
t_large = egg(M, T_0, T_y)
print 'At room temperature: to soft boil a small egg cook for %.2f minutes and a large egg for %.2f minutes.' % (t_small/60, t_large/60)
